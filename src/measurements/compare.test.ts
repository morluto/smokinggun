...
import type {MeasurementArtifactV1, ScalingAnalysisV2, ScalingAnalysisV3} from "../protocol/index.js";

it("classifies only same-kind artifacts from the same benchmark plan", () => {
  ...
  expect(
    classifyComparableMeasurementArtifacts(baseline, {...candidate, benchmarkDigest: "b".repeat(64)}),
  ).toMatchObject({code: "benchmark-mismatch"});
...
it("blocks promotion when imported measurements bind different shared input sets", () => {
  const baseline = measurement();
  const candidate = {
    ...measurement(),
    id: `meas_${"d".repeat(16)}` as const,
    samplesMs: [0.5],
    medianMs: 0.5,
    meanMs: 0.5,
...
it("blocks promotion when imported measurements bind different runtimes", () => {
  const baseline = measurement();
  const candidate = {
    ...measurement(),
    samplesMs: [0.5],
    ...
    isolation: {
      ...
      runtime: {name: "fixture-runner", version: "1.0.0", digest: "f".repeat(64)},
    },
  };
  const result = buildMeasurementComparison(baseline, candidate, "baseline.json", "candidate.json", [
    "a".repeat(64),
  ...
  expect(result.promotion).toBe("blocked");
  expect(result.promotionReasons).toContain("runtime-identity-mismatch");
});

it("blocks promotion across different measurement hosts", () => {
  const baseline = measurement();
  const candidate = {
    ...measurement(),
    samplesMs: [0.5],
    ...
    meanMs: 0.5,
    quartiles: {q1Ms: 0.5, q3Ms: 0.5},
    isolation: {...measurement().isolation, hostDigest: "0".repeat(64)},
  };
  const result = buildMeasurementComparison(baseline, candidate, "baseline.json", "candidate.json", [
    "a".repeat(64),
    "b".repeat(64),
...
it("never treats a failed scaling point as a zero-time improvement", () => {
  const baseline = scaling("base", [10]);
  const candidate = scaling("candidate", [5]);
  candidate.points[0] = {
    ...candidate.points[0]!,
    status: "failed",
    samplesMs: [],
  ...
  expect(result.improvement).toBe(false);
  expect(result.promotion).toBe("blocked");
  expect(result.promotionReasons).toContain("scaling-points-incomplete");
});

it("never treats a timed-out multi-scaling coordinate as a zero-time improvement", () => {
  const baseline = multiScaling("base", 10);
  const candidate = multiScaling("candidate", 5);
  candidate.points[0] = {
    ...candidate.points[0]!,
    status: "timed-out",
    samplesMs: [],
  ...
  const result = buildMultiScalingComparison(baseline, candidate, "baseline.json", "candidate.json", [
    "a".repeat(64),
    "b".repeat(64),
  ]);
  expect(result.improvement).toBe(false);
...
function measurement(): MeasurementArtifactV1 {
  return {
    ...
    reproduction: {
      ...
      executable: {path: "benchmark", digest: "b".repeat(64)},
...
function multiScaling(id: string, medianMs: number): ScalingAnalysisV3 {
  const coordinates = {paths: 1, terms: 1};
  return {
