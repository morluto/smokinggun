...
it("keeps finding identities stable across repeated scans", () => {
  fc.assert(fc.property(fc.array(fc.string({minLength: 0, maxLength: 60}), {maxLength: 12}), (lines) => {
...
it("keeps structural findings uniquely ordered and serializable for arbitrary source", () => {
  fc.assert(fc.property(fc.array(fc.constantFrom("for (const item of items) { items.includes(item); }", "while (ready) { values.sort(); }", "const value = 'for (x of y)'", "// for (x of y)"), {maxLength: 20}), (lines) => {
    const findings = scanSource("src/fixture.ts", lines.join("\n")).findings;
    const keys = findings.map((finding) => `${finding.location.path}\0${finding.location.startLine}\0${finding.ruleId}`);
    expect(new Set(keys).size).toBe(keys.length);
    expect(findings).toEqual([...findings].sort((left, right) => comparePortable(left.location.path, right.location.path) || left.location.startLine - right.location.startLine || comparePortable(left.ruleId, right.ruleId)));
...
it("normalizes repository paths without introducing parent traversal", () => {
  fc.assert(fc.property(fc.array(fc.constantFrom("src", "nested", "fixture.ts", ".", ".."), {minLength: 1, maxLength: 8}), fc.boolean(), (parts, windowsSeparators) => {
