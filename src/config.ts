...
export const outputFormats = ["human", "json", "markdown", "sarif"] as const;
...
export async function loadConfig(overrides: CliOverrides, environment: NodeJS.ProcessEnv = process.env): Promise<RuntimeConfig | ConfigFailure> {
  ...
  try {
    configPath = explicitPath === undefined ? await findNearestConfig(initialCwd, environment) : resolve(initialCwd, explicitPath);
  } catch (cause: unknown) {
    return configFailure("config-discovery-failed", "Could not inspect configuration locations.", cause instanceof Error ? cause.message : "Unknown configuration discovery failure.");
  }
  ...
  const fileCwd = fileValues.cwd === undefined ? initialCwd : resolve(configPath === undefined ? initialCwd : dirname(configPath), fileValues.cwd);
  ...
  const outputBase = overrides.output !== undefined || environment.FOOTGUN_OUTPUT !== undefined
    ? initialCwd
  ...
  const normalized: RuntimeConfig = {
    ...
    adapters: [...(merged.adapters ?? [])].map((value) => resolve(configPath === undefined ? initialCwd : dirname(configPath), value)).sort(comparePortable),
...
async function readJsonConfig(path: string): Promise<{readonly _tag: "ok"; readonly value: FileConfig} | ConfigFailure> {
  try {
    const raw = await readFile(path, "utf8");
    const parsed: unknown = JSON.parse(raw);
    const result = fileConfigSchema.safeParse(parsed);
    if (!result.success) {
      return configFailure("invalid-config", `Configuration at ${path} is invalid.`, result.error.issues.map((issue) => `${issue.path.join(".")}: ${issue.message}`).join("; "));
    }
...
function parseEnvironment(environment: NodeJS.ProcessEnv): FileConfig | ConfigFailure {
  ...
  if (environment.FOOTGUN_FORMAT !== undefined) {
    if (!isOutputFormat(environment.FOOTGUN_FORMAT)) return configFailure("invalid-environment", "FOOTGUN_FORMAT is invalid.", "Expected one of " + outputFormats.join(", ") + ".");
  ...
  for (const [name, key] of [["FOOTGUN_NO_COLOR", "noColor"], ["FOOTGUN_QUIET", "quiet"], ["FOOTGUN_DEBUG", "debug"], ["FOOTGUN_NON_INTERACTIVE", "nonInteractive"], ["FOOTGUN_STRICT", "strict"]] as const) {
    const value = environment[name];
    if (value !== undefined) {
      if (value !== "true" && value !== "false") return configFailure("invalid-environment", `${name} is invalid.`, "Use the literal string true or false.");
      values[key] = value === "true";
    }
  }
  if (environment.FOOTGUN_MAX_FINDINGS !== undefined) {
    const value = Number(environment.FOOTGUN_MAX_FINDINGS);
    if (!Number.isInteger(value) || value <= 0) return configFailure("invalid-environment", "FOOTGUN_MAX_FINDINGS is invalid.", "Use a positive integer.");
    values.maxFindings = value;
  }
  if (environment.FOOTGUN_EXCLUDE !== undefined) values.exclude = environment.FOOTGUN_EXCLUDE.split(",").filter((value) => value.length > 0);
  if (environment.FOOTGUN_ADAPTERS !== undefined) values.adapters = environment.FOOTGUN_ADAPTERS.split(",").filter((value) => value.length > 0);
  return values;
}
...
export function userDataDirectory(environment: NodeJS.ProcessEnv = process.env): string {
  return environment.FOOTGUN_DATA_DIR ?? resolve(environment.XDG_DATA_HOME ?? join(homedir(), ".local", "share"), "footgun");
}
