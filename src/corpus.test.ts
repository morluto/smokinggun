...
it("meets the labeled corpus precision and recall gate for the shipped rules", async () => {
  const cases = z.array(z.strictObject({language: z.string().min(1), path: z.string().min(1), expectedStructural: z.array(z.string()), expectedSemantic: z.array(z.string())})).parse(JSON.parse(await readFile(resolve(corpus, "labels.json"), "utf8")));
  ...
  for (const testCase of cases) {
    ...
    const structural = (treeStructural.coverage.status === "complete" ? treeStructural.findings : scanSource(testCase.path, source).findings).map((finding) => finding.ruleId);
    const semantic = scanTypeScript(corpus, [file]).findings.map((finding) => finding.ruleId);
    if (testCase.language === "python") semantic.push(...(await scanPythonSemantic(testCase.path, source)).findings.map((finding) => finding.ruleId));
    ...
    for (const rule of expected) actual.has(rule) ? current.truePositives += 1 : current.falsePositives += 1;
  ...
  for (const [language, current] of metrics) {
    const precision = current.truePositives + current.falsePositives === 0 ? 1 : current.truePositives / (current.truePositives + current.falsePositives);
...
it("records complete parse coverage for every shipped grammar fixture", async () => {
  const fixtures = [
    "c/canonical.c", "c/positive.c", "cpp/canonical.cpp", "cpp/positive.cpp", "csharp/Canonical.cs", "csharp/Positive.cs", "go/canonical.go", "go/positive.go", "java/Canonical.java", "java/Positive.java",
    "javascript/comments-and-strings.js", "javascript/positive.js", "kotlin/Canonical.kt", "kotlin/Positive.kt", "php/canonical.php", "php/positive.php", "python/membership-in-loop.py",
    "ruby/canonical.rb", "ruby/positive.rb", "rust/canonical.rs", "rust/positive.rs", "swift/canonical.swift", "swift/Positive.swift", "typescript/negative.ts", "typescript/nested-scan.ts", "typescript/positive.ts",
  ];
  const results = await Promise.all(fixtures.map(async (path) => parseWithTreeSitter(path, await readFile(resolve(corpus, path), "utf8"))));
