...
describe("SARIF import boundary", () => {
    ...
    const result = importSarif({
      ...
      runs: [{
        ...
        results: [{
          ...
          locations: [{physicalLocation: {artifactLocation: {uri: "src/example.py"}, region: {startLine: 4, startColumn: 2}}}],
          relatedLocations: [{physicalLocation: {artifactLocation: {uri: "src/other.py"}}}],
          fixes: [{description: {text: "untrusted fix"}}],
        }],
      }],
    }, "/repo", "a".repeat(64), "artifact://sha256/" + "b".repeat(64));
    expect("code" in result).toBe(false);
    if ("code" in result) return;
    expect(result.findings[0]).toMatchObject({scanner: "sarif:semgrep", scannerVersion: "1.2.3", ruleId: "python.lang.correctness", evidence: ["sarif:{\"primary\":\"stable-fingerprint\"}"], thirdParty: {properties: {originalRule: "python.lang.correctness", untrustedValue: {nested: true}}}});
  ...
  it("rejects absolute locations outside the repository and malformed versions without throwing", () => {
    const outside = importSarif({version: "2.1.0", runs: [{tool: {driver: {name: "tool"}}, results: [{locations: [{physicalLocation: {artifactLocation: {uri: "/other/file.ts"}}}]}]}]}, "/repo", "a".repeat(64));
    expect("code" in outside).toBe(false);
    if (!("code" in outside)) expect(outside.diagnostics.map((diagnostic) => diagnostic.code)).toContain("sarif-path-outside-root");
