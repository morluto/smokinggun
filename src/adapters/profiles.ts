...
export function importPprof(input: Uint8Array, options: ProfileImportOptions): ProfileSummaryV1 | ProblemV1 {
  try {
    ...
    const sampleTypes = profile.sampleType.map((sampleType) => ({
      ...
      unit: stringAt(strings, numberValue(sampleType.unit)),
    }));
    const functionNames = new Map<number, string>();
    for (const functionValue of profile.function)
      functionNames.set(numberValue(functionValue.id), stringAt(strings, numberValue(functionValue.name)));
    ...
    const result: ProfileSummaryV1 = {
      ...
      topFunctions,
      limitations: [
        "The summary preserves sampled values and symbol names; it does not replace pprof's full call graph or label semantics.",
      ],
    };
    ...
    return parsed.success
      ? parsed.data
      : problem(
          "invalid-pprof-summary",
          "The decoded pprof profile did not satisfy the normalized summary contract.",
...
export function importPerfettoSummary(input: unknown, options: ProfileImportOptions): TraceSummaryV1 | ProblemV1 {
  ...
  return checked.success
    ...
    : problem(
        "invalid-perfetto-summary",
        "The trace summary exceeded the normalized scalar-row contract.",
        "Limit query columns and values to strings, finite numbers, booleans, or null.",
      );
}
...
export async function importPerfettoTrace(options: PerfettoTraceOptions): Promise<TraceSummaryV1 | ProblemV1> {
  ...
  try {
    const result = await execa(
      executable,
      [...(options.executableArgs ?? []), "query", options.tracePath, options.query],
      {
        reject: false,
        stdin: "ignore",
    ...
    if (result.isCanceled || options.signal?.aborted === true)
      return problem(
        "trace-processor-cancelled",
        "The Perfetto trace processor was cancelled.",
        "Rerun the trace import when the local processor is available.",
      );
    if (result.timedOut)
      return problem(
        "trace-processor-timeout",
        "The Perfetto trace processor exceeded its timeout.",
        "Use a narrower query or increase the bounded timeout deliberately.",
      );
    if (result.exitCode !== 0)
      return problem(
        "trace-processor-failed",
        "The Perfetto trace processor returned a nonzero status.",
        "Inspect the trace processor diagnostics and verify the trace and query.",
      );
    ...
    const summary: TraceSummaryV1 = {
      ...
      rows: rows.rows,
      limitations: [
        "The summary preserves only the bounded CSV query result; raw .pftrace packets and unqueried slices remain in the source artifact.",
      ],
    };
    ...
    return checked.success
      ...
      : problem(
          "invalid-perfetto-summary",
          "The trace processor result exceeded the normalized scalar-row contract.",
          "Limit query columns and scalar values.",
        );
  } catch (cause: unknown) {
    return problem(
      "trace-processor-unavailable",
...
function parseCsvRows(
  input: string,
): {readonly columns: string[]; readonly rows: Array<Record<string, string | number | boolean | null>>} | ProblemV1 {
  try {
    ...
    if (!Array.isArray(records) || records.length > 1000)
      return problem(
        "trace-processor-output-too-large",
        "The Perfetto trace processor returned more than 1,000 rows.",
        "Add a LIMIT of at most 1,000 rows to the query.",
      );
    ...
    for (const record of records) {
      ...
      if (!parsed.success)
        return problem(
          "invalid-perfetto-csv",
          "The trace processor returned unsupported CSV scalar values.",
          "Select only string, numeric, boolean, or null-compatible columns.",
        );
    ...
    return problem(
      "invalid-perfetto-csv",
      "The trace processor did not return valid bounded CSV.",
      cause instanceof Error ? cause.message : "Check the trace processor version and query output.",
    );
  }
}
