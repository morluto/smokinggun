...
export function importBenchmark(input: unknown, options: BenchmarkImportOptions): BenchmarkImportV2 | ProblemV1 {
  const records =
    options.tool === "hyperfine"
      ...
      : options.tool === "pyperf"
        ? importPyperf(input, options)
        : options.tool === "google-benchmark"
          ? importGoogleBenchmark(input, options)
          : options.tool === "criterion"
            ? importCriterion(input, options)
  ...
  const result = {
    ...
    ...(options.rawArtifactDigest === undefined ? {} : {rawArtifactDigest: options.rawArtifactDigest}),
  };
  const parsed = Protocol.benchmarkImport.safeParse(result);
  return parsed.success
    ? parsed.data
    : problem(
        "invalid-benchmark-import",
        "The normalized benchmark result did not satisfy its protocol contract.",
        "Check the source benchmark JSON and importer version.",
      );
}
...
function importHyperfine(input: unknown, options: BenchmarkImportOptions): BenchmarkRecordV2[] | ProblemV1 {
  ...
  for (const [index, value] of results.entries()) {
    ...
    if (entry === undefined || typeof entry.command !== "string" || (samples === undefined && mean === undefined))
      continue;
    const normalized =
      samples === undefined ? [mean === undefined ? 0 : mean * 1000] : samples.map((sample) => sample * 1000);
    records.push(
      makeRecord(options, entry.command, normalized, "s", {
        index,
        ...(samples === undefined ? {summaryOnly: true} : {}),
      }),
    );
  }
  return records.length === 0
    ? problem(
        "invalid-hyperfine",
        "No usable Hyperfine benchmark result was found.",
        "Each result needs a command and times or mean in seconds.",
      )
    : records;
}
...
function importPyperf(input: unknown, options: BenchmarkImportOptions): BenchmarkRecordV2[] | ProblemV1 {
  ...
  for (const [index, value] of benchmarks.entries()) {
    ...
    if (normalized.length > 0)
      records.push(
        makeRecord(options, entry.name, normalized, "s", {index, ...(samples.length === 0 ? {summaryOnly: true} : {})}),
      );
  }
  return records.length === 0
    ? problem(
        "invalid-pyperf",
        "No usable pyperf benchmark values were found.",
        "Each benchmark needs run values or a mean expressed in seconds.",
      )
    : records;
}
...
function importCriterion(input: unknown, options: BenchmarkImportOptions): BenchmarkRecordV2[] | ProblemV1 {
  ...
  if (observed === undefined)
    return problem(
      "invalid-criterion",
      "The input is not a Criterion estimates JSON artifact.",
      "Pass a Criterion estimates.json file containing mean or median point_estimate values.",
    );
  const record = makeRecord(options, "criterion", [observed / 1_000_000], "ns", {summaryOnly: true});
  return [
    {
      ...record,
      medianMs: (median ?? observed) / 1_000_000,
      meanMs: mean === undefined ? observed / 1_000_000 : mean / 1_000_000,
    },
  ];
}
...
function importJmh(input: unknown, options: BenchmarkImportOptions): BenchmarkRecordV2[] | ProblemV1 {
  ...
  return records.length === 0
    ? problem(
        "invalid-jmh",
        "No usable JMH primary metrics were found.",
        "Each record needs benchmark, primaryMetric.scoreUnit, and score or rawData.",
      )
    : records;
}
  ...
  const digest = createHash("sha256")
    .update(
      stableJson({
        ...
        rawArtifactDigest: options.rawArtifactDigest,
        metadata,
      }),
    )
    .digest("hex");
