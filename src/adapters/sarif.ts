...
const sarifResult = z.looseObject({
  ruleId: z.string().optional(),
  level: z.string().optional(),
  message: z.looseObject({text: z.string().optional()}).optional(),
  locations: z.array(sarifLocation).optional(),
  fingerprints: z.record(z.string(), z.string()).optional(),
  properties: z.record(z.string(), z.unknown()).optional(),
  relatedLocations: z.array(sarifLocation).optional(),
  fixes: z.array(z.unknown()).optional(),
});
const sarifInvocation = z.looseObject({
  executionSuccessful: z.boolean().optional(),
  toolExecutionNotifications: z.array(z.unknown()).optional(),
});
const sarifRun = z.looseObject({
  tool: z.looseObject({driver: z.looseObject({name: z.string().optional(), version: z.string().optional()})}),
  results: z.array(sarifResult).optional(),
  invocations: z.array(sarifInvocation).optional(),
  properties: z.record(z.string(), z.unknown()).optional(),
});
    ...
    for (const [resultIndex, result] of (run.results ?? []).entries()) {
      ...
      findings.push({
        ...
        location: {
          path: portablePath,
          startLine: line,
          startColumn: Math.max(0, (region?.startColumn ?? 1) - 1),
        ...
        evidence: [
          result.fingerprints === undefined
            ...
            : `sarif:${JSON.stringify(result.fingerprints)}`,
        ],
        complexity: {},
