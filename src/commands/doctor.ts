...
export default class Doctor extends BaseCommand {
  ...
  static override flags = {...globalFlags, "check-updates": Flags.boolean({description: "Perform an explicit bounded npm registry check.", default: false}), "probe-isolation": Flags.boolean({description: "Probe optional local isolation executables.", default: false})};
  ...
  public async run(): Promise<void> {
    ...
    const registry = parsed.flags["check-updates"] ? await checkRegistry(context.config.cwd, context.signal) : {state: "not-requested" as const};
    const isolation = parsed.flags["probe-isolation"] ? await probeIsolation(context.signal) : [];
    const scanners = context.scannerRegistry();
    const hostControls = {
      processTree: {status: "best-effort", mechanism: "execa cleanup and forceKillAfterDelay", limitation: process.platform === "win32" ? "Windows job-object accounting is not asserted by the host runner." : "Child-process cleanup is delegated to the host runner."},
    ...
    const isolationSummary = isolation.length === 0 ? "not requested" : isolation.map((entry) => `${entry.backend}: ${entry.available ? "available" : entry.reason}`).join(", ");
    const scannerSummary = scanners.map((scanner) => `${scanner.id}: ${scanner.availability}`).join(", ");
    const human = [`Footgun 1.0.0`, `Node ${result.node} on ${result.platform}`, `Configuration: ${result.configSource}`, `Network checks: ${registry.state}`, `Scanners: ${scannerSummary}`, `Tree-sitter runtime: ${treeSitter.runtime}; grammars: ${treeSitter.grammars} (${treeSitter.languages.length} loaded)`, `Isolation probes: ${isolationSummary}`].join("\n");
...
async function checkRegistry(cwd: string, signal: AbortSignal): Promise<{readonly state: "available" | "unavailable"; readonly version?: string; readonly integrity?: string}> {
  try {
    const result = await execa("npm", ["view", "footgun", "version", "dist.integrity", "--json"], {cwd, reject: false, stdin: "ignore", ...(signal === undefined ? {} : {cancelSignal: signal}), timeout: 5_000});
