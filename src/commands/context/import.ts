...
export default class ContextImport extends BaseCommand {
  ...
  static override flags = {...globalFlags, investigation: Flags.string({description: "Attach imported context to an existing investigation bundle."})};
  ...
  public async run(): Promise<void> {
    ...
    const value = {schemaVersion: "footgun.context-import.v1", state: result.state, ...(result.index === undefined ? {} : {index: result.index}), diagnostics: result.diagnostics};
    const human = [`SCIP import: ${result.state}`, result.index === undefined ? "No semantic index was produced." : `Files indexed: ${result.index.coverage.filesIndexed}/${result.index.coverage.filesDiscovered}`, `Definitions: ${result.index?.definitions.length ?? 0}`, `References: ${result.index?.references.length ?? 0}`, ...(result.diagnostics.length === 0 ? [] : [`Diagnostics: ${result.diagnostics.length}`])].join("\n");
    ...
    if (parsed.flags.investigation !== undefined && result.index !== undefined) {
      ...
      if (stored !== undefined) {
        ...
        await recordInvestigationSnapshot(context.artifacts, {...stored.bundle, state: "context-resolved", reports: [...new Set([...stored.bundle.reports, artifact])].sort(comparePortable), evidence: [...stored.bundle.evidence, {schemaVersion: "footgun.evidence.v1", id: `${parsed.flags.investigation}:context:${result.index.digest.slice(0, 16)}`, kind: "context", claimClass: "static-fact", summary: "Imported SCIP repository context", artifact, digest: result.index.digest}]});
      }
    }
