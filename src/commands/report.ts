...
export default class Report extends BaseCommand {
  ...
  static override flags = {...globalFlags, benchmark: Flags.string({description: "Interpret the artifact as standard benchmark JSON.", options: ["hyperfine", "pyperf", "google-benchmark", "criterion", "jmh"]}), profile: Flags.string({description: "Interpret the artifact as a pprof or Perfetto summary/trace." , options: ["pprof", "perfetto"]}), "trace-query": Flags.string({description: "Bounded Perfetto SQL query for a raw trace; defaults to a 1,000-row slice query.", default: "SELECT ts, dur, name FROM slice LIMIT 1000"}), investigation: Flags.string({description: "Attach the rendered artifact to an investigation bundle."})};
  ...
  public async run(): Promise<void> {
    ...
    try {
      if (parsed.flags.benchmark !== undefined && parsed.flags.profile !== undefined) this.emitProblem({schemaVersion: "footgun.problem.v1", code: "report-input-kind-conflict", message: "Choose one external artifact kind for report.", recovery: "Pass either --benchmark or --profile, not both."}, 2, context);
      ...
      const input: unknown = parsed.flags.profile === "pprof" || (parsed.flags.profile === "perfetto" && !looksLikeJson(raw)) ? undefined : JSON.parse(raw);
      if (parsed.flags.benchmark !== undefined) {
        const tool = parsed.flags.benchmark;
        if (!isBenchmarkTool(tool)) this.emitProblem({schemaVersion: "footgun.problem.v1", code: "invalid-benchmark-tool", message: "The benchmark tool is not supported by this Footgun build.", recovery: "Choose hyperfine, pyperf, google-benchmark, criterion, or jmh."}, 2, context);
        const benchmark = importBenchmark(input, {tool, rawArtifact: artifactReference, rawArtifactDigest: storedArtifact.digest});
        if ("code" in benchmark) this.emitProblem(benchmark, 2, context);
        await printResult(benchmark, `Imported ${benchmark.tool} benchmark records: ${benchmark.records.length}`, context);
      ...
      if (parsed.flags.profile !== undefined) {
        ...
        if (parsed.flags.profile === "pprof") {
          ...
          await printResult(profile, `Imported pprof profile: ${profile.sampleCount} samples, ${profile.topFunctions.length} top functions`, context);
          ...
          const trace = input === undefined
            ? await importPerfettoTrace({sourceArtifact, sourceDigest, tracePath: resolve(context.config.cwd, parsed.args.artifact), query: parsed.flags["trace-query"]})
            : importPerfettoSummary(input, {sourceArtifact, sourceDigest});
          if ("code" in trace) this.emitProblem(trace, trace.code === "trace-processor-unavailable" || trace.code === "trace-processor-timeout" ? 3 : 2, context);
      ...
      const parsedReport = "_tag" in scanReport && isSarifDocument(input)
      ...
      if (context.signal.aborted) this.emitProblem({schemaVersion: "footgun.problem.v1", code: "cancelled", message: "The report operation was cancelled.", recovery: "Rerun the report command when the artifact is available."}, 130, context);
      const message = cause instanceof Error ? cause.message : "The artifact could not be read.";
      this.emitProblem({schemaVersion: "footgun.problem.v1", code: "artifact-read-failed", message, recovery: "Pass a JSON ScanReportV1 artifact path."}, 2, context);
    }
  }
}
...
async function recordReportedArtifact(context: RuntimeContext, investigationId: string | undefined, artifact: string, kind: "static" | "benchmark" | "profile" | "trace"): Promise<void> {
  if (investigationId === undefined) return;
  const stored = await loadLatestInvestigation(context.artifacts, investigationId);
  if (stored === undefined) return;
  const claimClass = kind === "benchmark" ? "constant-factor" as const : kind === "profile" || kind === "trace" ? "system-bottleneck" as const : "static-fact" as const;
  const next = {...stored.bundle, state: "reported" as const, reports: [...new Set([...stored.bundle.reports, artifact])].sort(comparePortable), evidence: [...stored.bundle.evidence, {schemaVersion: "footgun.evidence.v1" as const, id: `${investigationId}:report:${artifact}`, kind, claimClass, summary: `Rendered ${kind} artifact`, artifact}]};
