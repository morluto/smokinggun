...
export default class Scan extends BaseCommand {
  ...
  static override flags = {...globalFlags, scanner: Flags.string({description: "Scanner IDs or auto; repeat for multiple backends.", multiple: true}), only: Flags.string({description: "Restrict analysis to language:<name>, extension, or path filters.", multiple: true}), adapter: Flags.string({description: "External adapter manifest path; repeat to configure adapters.", multiple: true})};
  ...
  public async run(): Promise<void> {
    ...
    try {
      ...
      if (context.config.strict && report.coverage.some((record) => record.parseStatus !== "complete")) {
        if (context.config.format === "human") this.emitProblem({schemaVersion: "footgun.problem.v1", code: "incomplete-coverage", message: "The scan completed with incomplete coverage.", recovery: "Inspect diagnostics or rerun without --strict."}, 3, context);
        this.exit(3);
      }
      ...
      if (context.signal.aborted) this.emitProblem({schemaVersion: "footgun.problem.v1", code: "cancelled", message: "The scan was cancelled.", recovery: "Rerun the scan when the repository is available."}, 130, context);
      const message = cause instanceof Error ? cause.message : "The scan failed unexpectedly.";
      this.emitProblem({schemaVersion: "footgun.problem.v1", code: "scan-failed", message, recovery: "Run with --debug for diagnostics."}, 1, context);
    }
  }
}
