...
export function scanTypeScript(root: string, files: ReadonlyArray<string>, signal?: AbortSignal): TypeScriptSemanticResult {
  ...
  try {
      ...
      const visit = (node: ts.Node, loopDepth: number): void => {
        ...
        if (loopDepth > 0 && ts.isCallExpression(node)) {
          ...
          if (name !== undefined && collectionOperationNames.has(name)) {
            ...
            const receiverType = receiver === undefined ? undefined : checker.typeToString(checker.getTypeAtLocation(receiver));
            const typed = receiverType !== undefined && collectionTypePattern.test(receiverType);
            findings.push(makeFinding(relativePath, sourceFile, node, typed ? "type-informed" : "candidate", typed ? `TypeScript checker resolved ${receiverType} before a ${name} call inside a loop.` : `Collection operation ${name} appears inside a loop; the receiver type is unresolved.`, typed ? "Precompute an index or combine the collection pass while preserving ordering and mutation semantics." : "Inspect the receiver type and consider a precomputed lookup or one-pass transformation.", [typed ? `receiver type: ${receiverType}` : "receiver type was unknown", `enclosing loop depth: ${loopDepth}`], "typescript-collection-operation-in-loop"));
          }
          if (name !== undefined && queryNames.has(name)) {
            findings.push(makeFinding(relativePath, sourceFile, node, "type-informed", `Potential ${name} call occurs inside a TypeScript loop.`, "Check for N+1 I/O or query behavior and batch only when authorization, ordering, errors, and caching remain equivalent.", [`call resolved as ${name}`, `enclosing loop depth: ${loopDepth}`], "typescript-call-in-loop"));
          }
        }
    ...
    const diagnostic: ProblemV1 = {schemaVersion: "footgun.problem.v1", code: "typescript-semantic-failed", message: "TypeScript semantic scanning was unavailable.", detail: cause instanceof Error ? cause.message : "Unknown TypeScript compiler failure.", recovery: "Inspect the TypeScript project configuration and rerun the scan."};
    return {state: "partial", diagnostics: [...indexResult.diagnostics, diagnostic], findings: dedupe(findings), ...(indexResult.index === undefined ? {} : {index: indexResult.index})};
  }
}

const collectionOperationNames = new Set(["find", "findIndex", "includes", "indexOf", "filter", "map", "reduce", "sort"]);
...
function makeFinding(path: string, sourceFile: ts.SourceFile, node: ts.Node, confidence: FindingV1["confidence"], message: string, suggestion: string, assumptions: ReadonlyArray<string>, ruleId: string): FindingV1 {
  const start = sourceFile.getLineAndCharacterOfPosition(node.getStart(sourceFile));
  const line = start.line + 1;
  const id = `fg_${createHash("sha256").update(`${path}\0${line}\0${ruleId}\0${node.getText(sourceFile)}`).digest("hex").slice(0, 16)}`;
  const location: LocationV1 = {path, startLine: line, startColumn: start.character, endLine: line, endColumn: start.character + Math.max(1, node.getWidth(sourceFile))};
  return {
