...
const languageByExtension: Readonly<Record<string, string>> = {
  ".c": "c", ".h": "c", ".cc": "cpp", ".cpp": "cpp", ".hpp": "cpp", ".cs": "csharp", ".go": "go", ".java": "java", ".js": "javascript", ".jsx": "javascript", ".mjs": "javascript", ".cjs": "javascript", ".kt": "kotlin", ".php": "php", ".py": "python", ".rb": "ruby", ".rs": "rust", ".swift": "swift", ".ts": "typescript", ".tsx": "typescript",
};

const manifestNames: Readonly<Record<string, string>> = {
  "package.json": "npm", "pnpm-lock.yaml": "pnpm", "yarn.lock": "yarn", "package-lock.json": "npm", "pyproject.toml": "python", "requirements.txt": "python", "Cargo.toml": "cargo", "go.mod": "go", "pom.xml": "maven", "build.gradle": "gradle", "build.gradle.kts": "gradle", "Gemfile": "bundler", "composer.json": "composer", "Package.swift": "swift", "CMakeLists.txt": "cmake",
};

export async function buildRepositoryInventory(root: string, sourceFiles: ReadonlyArray<string>, ignored: ReadonlyArray<string>): Promise<RepositoryInventoryV1> {
  ...
  const tests = relativeFiles.filter((file) => /(?:^|\/)(?:test|tests|spec|specs|__tests__)(?:\/|\.)|(?:\.test|\.spec)\.[^.]+$/i.test(file));
  const benchmarks = relativeFiles.filter((file) => /(?:^|\/)(?:bench|benchmark|benchmarks)(?:\/|\.)|(?:\.bench|\.benchmark)\.[^.]+$/i.test(file));
  ...
  const valueWithoutDigest = {
    ...
    languages: [...languageCounts.entries()].sort(([left], [right]) => comparePortable(left, right)).map(([language, value]) => ({language, files: value.files, extensions: [...value.extensions].sort(comparePortable)})),
