            ...
            invocations: [
              {
                ...
                toolExecutionNotifications: [
                  {level: "error", message: {text: safeProblem.message}, properties: safeProblem},
                ],
              },
            ],
...
function normalizeFlags(flags: ParsedGlobalFlags): GlobalFlags {
  ...
  const booleanValue = (name: string): boolean | undefined =>
    typeof input[name] === "boolean" ? input[name] : undefined;
  const numberValue = (name: string): number | undefined => (typeof input[name] === "number" ? input[name] : undefined);
  const cwd = stringValue("cwd");
  if (cwd !== undefined) normalized.cwd = cwd;
  const config = stringValue("config");
  if (config !== undefined) normalized.config = config;
  const format = stringValue("format");
  if (format === "human" || format === "json" || format === "markdown" || format === "sarif")
    normalized.format = format;
  const output = stringValue("output");
  if (output !== undefined) normalized.output = output;
  const noColor = booleanValue("no-color") ?? booleanValue("noColor");
  if (noColor !== undefined) normalized.noColor = noColor;
  const quiet = booleanValue("quiet");
  if (quiet !== undefined) normalized.quiet = quiet;
  const debug = booleanValue("debug");
  if (debug !== undefined) normalized.debug = debug;
  const nonInteractive = booleanValue("non-interactive") ?? booleanValue("nonInteractive");
  if (nonInteractive !== undefined) normalized.nonInteractive = nonInteractive;
  const strict = booleanValue("strict");
  ...
  const maxFindings = numberValue("max-findings") ?? numberValue("maxFindings");
  if (maxFindings !== undefined) normalized.maxFindings = maxFindings;
  return normalized;
}
