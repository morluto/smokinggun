...
for (const testCase of cases) {
  ...
  const structural = new Set((treeStructural.coverage.status === "complete" ? treeStructural.findings : scanSource(testCase.path, source).findings).map((finding) => finding.ruleId));
  const semantic = new Set(scanTypeScript(root, [resolve(root, testCase.path)]).findings.map((finding) => finding.ruleId));
  if (testCase.language === "python") for (const finding of (await scanPythonSemantic(testCase.path, source)).findings) semantic.add(finding.ruleId);
...
for (const path of [...fixtures, "labels.json"].sort()) corpusDigest.update(path).update("\0").update(await readFile(resolve(root, path))).update("\0");
const parseResults = await Promise.all(fixtures.map(async (path) => parseWithTreeSitter(path, await readFile(resolve(root, path), "utf8"))));
...
const rates = (metric) => ({...metric, precision: metric.truePositives + metric.falsePositives === 0 ? 1 : metric.truePositives / (metric.truePositives + metric.falsePositives), recall: metric.expectedPositives === 0 ? 1 : metric.truePositives / metric.expectedPositives});
const summary = {corpusRevision: corpusDigest.digest("hex"), overall: {precision, recall, truePositives, falsePositives, expectedPositives}, perLanguage: Object.fromEntries(Object.entries(perLanguage).map(([key, value]) => [key, rates(value)])), perRule: Object.fromEntries(Object.entries(perRule).map(([key, value]) => [key, rates(value)])), parseCoverage: {complete: parseResults.filter((result) => result.status === "complete").length, total: parseResults.length}};
console.log(JSON.stringify(summary, null, 2));
const metricFailures = [...Object.entries(summary.perLanguage), ...Object.entries(summary.perRule)].flatMap(([name, metric]) => [
...
if (precision < 0.9 || recall < 0.8 || parseRate < 0.99 || metricFailures.length > 0) {
  console.error(JSON.stringify({code: "corpus-threshold-failed", failures: [...metricFailures, ...(precision < 0.9 ? [`overall precision ${precision.toFixed(3)} < 0.900`] : []), ...(recall < 0.8 ? [`overall recall ${recall.toFixed(3)} < 0.800`] : []), ...(parseRate < 0.99 ? [`parse coverage ${parseRate.toFixed(3)} < 0.990`] : [])]}));
  process.exitCode = 1;
}
