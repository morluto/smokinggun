...
if (Object.keys(packageJson.bin ?? {}).join(",") !== "footgun") failures.push("package exposes more than the footgun executable");
...
for (const required of ["dist/bin/footgun.js", "grammar.lock.json", "skill/SKILL.md", ...schemaFiles.map((file) => `schemas/${file}`)]) {
...
if (failures.length > 0) {
  ...
  console.log(JSON.stringify({name: packageJson.name, version: packageJson.version, files: paths.length, compressedBytes: pack.size, unpackedBytes: pack.unpackedSize}, null, 2));
}
